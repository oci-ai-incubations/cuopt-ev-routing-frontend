    ...
    cities: [
      {
        id: 'mumbai',
        name: 'Mumbai',
        coordinates: { lat: 19.05, lng: 72.88 },
        defaultZoom: 12,
        serviceRadius: 15,
...
export function generateLicensePlate(countryCode: string, index: number): string {
  ...
  switch (countryCode) {
    case 'GB': {
      ...
      const letters2 = ['YRK', 'FLT', 'ABC', 'XYZ', 'JKL', 'MNO', 'PQR', 'STU', 'VWX', 'DEF'][
        index % 10
      ];
    ...
    case 'FR': {
      ...
      const l2 =
        String.fromCharCode(65 + ((index + 2) % 26)) + String.fromCharCode(65 + ((index + 3) % 26));
