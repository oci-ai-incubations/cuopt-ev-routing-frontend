...
export function useDashboardOptimization() {
  ...
  const handleRunParallel = useCallback(async () => {
    ...
    try {
      ...
      const payloads = clusters.map((cluster) => {
        ...
        const vehicles = Array.from({ length: vehiclesPerCluster }, (_, i) => {
          ...
          return {
            id: i,
            capacity: config.vehicleCapacity,
            startLat: config.enableHomeStart
              ? clusterCenterLat + (Math.random() - 0.5) * homeOffset * 2
              : 0,
            startLng: config.enableHomeStart
              ? clusterCenterLng + (Math.random() - 0.5) * homeOffset * 2
              : 0,
          };
        });
      ...
      addToast({
        ...
        message: `Completed ${clusters.length} clusters with ${results
          .filter(Boolean)
          .reduce((sum, r) => sum + (r?.num_vehicles || 0), 0)} vehicles`,
      });
    } catch (error) {
      addToast({
        type: 'error',
  ...
  }, [stops, config, clearParallelJobs, updateParallelJob, setResult, setIsOptimizing, addToast]);
