...
import type { SSOAuthorizeResponse, SSOProvider, User } from '@/types/auth';
...
describe('Login page', () => {
  ...
  it('renders SSO buttons when fetchPublicProviders returns providers', async () => {
    ...
    expect(await screen.findByRole('button', { name: /sign in with oracle idcs/i })).toBeInTheDocument();
  ...
  it('clicking an SSO button stores state and triggers redirect', async () => {
    ...
    // jsdom: spy on the location assignment.
    ...
    try {
      renderLogin();
      const button = await screen.findByRole('button', { name: /sign in with oracle idcs/i });
