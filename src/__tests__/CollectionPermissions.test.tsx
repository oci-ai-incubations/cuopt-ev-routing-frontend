...
import type { AdminUser, CollectionPermission } from '@/types/admin';
...
const PERMS: CollectionPermission[] = [
  { id: 100, user_id: 2, collection_id: 'cuopt-routes', permission_level: 'read', user_email: 'r@e.com' },
];
...
describe('CollectionPermissions', () => {
  it('loads collection permissions after entering an ID', async () => {
    ...
    await waitFor(() =>
      expect(listCollectionPermissions).toHaveBeenCalledWith('cuopt-routes'),
    );
  ...
  it('revokes an existing permission', async () => {
    ...
    await waitFor(() =>
      expect(deleteCollectionPermission).toHaveBeenCalledWith('cuopt-routes', 2),
    );
  });
});
