...
function getEvMetadata(metadata?: Record<string, unknown>): EvStopMetadata | null {
  ...
  const connectorType =
    typeof metadata.connectorType === 'string' ? metadata.connectorType : undefined;
...
export function RouteMap({ isActive = true }: RouteMapProps) {
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-2">
        ...
        {showStyleSelector && (
          ...
          >
            ...
            <div
              className={`border-t px-3 py-2 ${isDarkStyle ? 'border-dark-border' : 'border-gray-100'}`}
            >
              <button
              ...
              {showTrafficInfo && (
                ...
                >
                  Real-time traffic data requires API integration (e.g., TomTom, HERE, Google). The
                  Streets map shows road types and importance for planning purposes.
                </div>
              )}
            </div>
          </div>
      ...
      {routes.length > 0 && (
        <div
          className={`absolute bottom-4 left-4 z-[1000] rounded-lg shadow-lg p-3 max-w-[200px] ${
            isDarkStyle
        ...
        >
          ...
          {(() => {
            ...
            if (unassignedCount > 0) {
              return (
                <div
                  className={`mt-2 pt-2 border-t ${isDarkStyle ? 'border-dark-border' : 'border-gray-200'}`}
                >
                  <div className="flex items-center gap-2">
                    <div
                      className="w-3 h-3 rounded-full bg-gray-400 opacity-50 border-2 border-red-500"
                      style={{ width: '12px', height: '12px' }}
                    />
                    <span
                      className={`text-[10px] ${isDarkStyle ? 'text-red-400' : 'text-red-500'}`}
                    >
      ...
      >
        ...
        {/* Depot marker */}
        {stops.length > 0 && (
          <Marker position={[stops[0].lat, stops[0].lng]} icon={createMarkerIcon('#76B900', true)}>
            <Popup>
                ...
                <div style={{ fontSize: '12px', color: '#666' }}>
                  ...
                  >
                    ...
                    <span style={{ fontFamily: 'monospace', color: '#333' }}>
                      {stops[0].lat.toFixed(5)}
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span>Longitude:</span>
                    <span style={{ fontFamily: 'monospace', color: '#333' }}>
                      {stops[0].lng.toFixed(5)}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {stops.slice(1).map((stop) => {
          ...
          const color = assignedRoute ? getVehicleColor(assignedRoute.vehicle_id).color : '#9CA3AF'; // Gray for unassigned
                ...
                <div style={{ fontFamily: 'system-ui, sans-serif', minWidth: '200px' }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', lineHeight: '1.6' }}>
                    {stop.postcode && (
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>Postcode:</span>
                        <span style={{ fontFamily: 'monospace', color: '#333', fontWeight: 500 }}>
                          {stop.postcode}
                        </span>
                      </div>
                    )}
