...
import { ApiKeysPanel } from '@/components';
...
describe('ApiKeysPanel', () => {
  ...
  it('shows "*** (set)" placeholders when keys are configured', async () => {
    ...
    render(<ApiKeysPanel />);
    await waitFor(() => expect(screen.getAllByPlaceholderText(/\*\*\* \(set/i)).toHaveLength(2));
  });
  ...
  it('only includes typed fields in PATCH payload', async () => {
    ...
    render(<ApiKeysPanel />);
    await waitFor(() => expect(screen.getAllByPlaceholderText('(not configured)')).toHaveLength(2));
  ...
  it('renders error when save fails', async () => {
    ...
    render(<ApiKeysPanel />);
    await waitFor(() => expect(screen.getAllByPlaceholderText('(not configured)')).toHaveLength(2));
    await userEvent.type(screen.getAllByPlaceholderText('(not configured)')[0], 'x');
    await userEvent.click(screen.getByRole('button', { name: /save/i }));
