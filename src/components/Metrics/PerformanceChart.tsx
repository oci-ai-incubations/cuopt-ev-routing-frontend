...
import { performanceBaselines, parallelExecutionData, getCountryByCode } from '@/data';
...
export function PerformanceChart() {
  ...
  const productivityMultiplier =
    solveTime > 0 ? Math.round(manualPlanningMinutes / (solveTime / 60)) : 0;
    ...
    <div className="h-full flex flex-col overflow-y-auto">
      ...
      {solveTime > 0 && stops.length > 0 && (
        <div className="mb-3 p-3 rounded-lg border bg-oracle-red/10 border-oracle-red/30">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              <div className="text-lg font-bold text-white">
                {Math.round(timeSavingsMinutes)} min
              </div>
          ...
          <p className="text-xs text-gray-500 mt-2 text-center">
            *Based on 4 optimizations/day, 250 days/year, {currencySymbol}
            {hourlyRate}/hr dispatcher cost
          </p>
        </div>
      )}
      ...
      {solveTime > 0 && (
        <div
          ...
          style={{
            backgroundColor: isBetterThanBaseline
              ? 'rgba(199, 70, 52, 0.1)'
              : 'rgba(59, 130, 246, 0.1)',
            borderColor: isBetterThanBaseline
              ? 'rgba(199, 70, 52, 0.3)'
              : 'rgba(59, 130, 246, 0.3)',
          }}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span
                ...
                style={{ color: isBetterThanBaseline ? '#C74634' : '#60A5FA' }}
              >
                {isBetterThanBaseline
          ...
          {showBaselineInfo && (
            <div className="mt-2 pt-2 border-t border-gray-700 text-xs text-gray-400 space-y-1">
              <p>
                <strong className="text-gray-300">Baseline:</strong> cuOPT constrained field service
                solve (time windows, service durations, capacity).
              </p>
              <p>
                <strong className="text-gray-300">Hardware:</strong> NVIDIA A10G GPU (24GB VRAM) via
                NIM microservice.
              </p>
              <p>
                <strong className="text-gray-300">Note:</strong> Complex constraints (tight time
                windows, high utilization) naturally require more solve time.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-dark-border flex-shrink-0">
        ...
        <ResponsiveContainer width="100%" height={120}>
          <BarChart data={parallelExecutionData}>
            ...
            />
            <YAxis stroke="#6B7280" tick={{ fill: '#9CA3AF', fontSize: 10 }} width={40} />
            <Tooltip
              contentStyle={{
                backgroundColor: '#1B1F2E',
                border: '1px solid #2D3748',
