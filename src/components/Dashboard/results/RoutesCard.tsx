        ...
        {routes.length === 0 ? (
          ...
          <div className="h-full min-h-0 flex flex-col">
            <div className="flex items-center justify-between text-xs text-gray-400 mb-2 shrink-0">
              <span>
                Showing {visibleRoutes.length} of {routes.length} routes
              </span>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-400 space-x-4">
                        ...
                        <span
                          className={
                            route.route_duration > shiftLimitMinutes ? 'text-yellow-400' : ''
                          }
                        >
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-3 border-t border-dark-border">
                        ...
                        {(() => {
                          ...
                          const totalDriveTime = segments
                            .filter((s) => s.type === 'drive')
                            .reduce((sum, s) => sum + s.duration, 0);
                          const totalJobTime = segments
                            .filter((s) => s.type === 'job')
                            .reduce((sum, s) => sum + s.duration, 0);
                              ...
                              <div className="flex items-center gap-4 text-xs">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span className="text-blue-400 font-medium">
                                    {formatDuration(totalDriveTime)}
                                  </span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <Wrench className="w-3 h-3 text-[#C74634]" />
                                  <span className="text-gray-400">Jobs:</span>
                                  <span className="text-[#C74634] font-medium">
                                    {formatDuration(totalJobTime)}
                                  </span>
                                </div>
                                <div className="text-gray-500">
                                  (
                                  {Math.round(
                                    (totalJobTime / (totalDriveTime + totalJobTime)) * 100,
                                  ) || 0}
                                  % productive)
                                </div>
                              </div>
